...
def main():
   ...
   tweets = getTweetsFromFile("tweets_small.txt")
   scores = getTweetScoresFromFile("scores_small.txt")
...
def getFeatureMatrixFromVecs(vectorList):
   ...
   for vec in vectorList[1:]:
      statusbar(1.0 * i / l, 'getFeatureMatrixFromVecs')
      i += 1
      featureMatrix = np.vstack([featureMatrix, vec])

   statusbar(1, 'getFeatureMatrixFromVecs')
   return featureMatrix
